    ...
    //   console.log("PhiloNodes extension chargée");

    await loadSettings();
    await checkBackendStatus();
    await loadHistory();

    const result = await chrome.storage.local.get("pendingRecommendation");
    if (result.pendingRecommendation) {
        const recommendation = result.pendingRecommendation;

        displayQuote(recommendation, {
        context: "Recommandation automatique",
        keywords: [],
        });

        chrome.storage.local.remove("pendingRecommendation");
    ...
    document
        ...
        .addEventListener("click", getQuoteForCurrentPage);
    const autoAnalysisToggle = document.getElementById("autoAnalysis");
    if (autoAnalysisToggle) {
        ...
        tabs.forEach((tab) => {
            chrome.tabs
            .sendMessage(tab.id, {
                action: enabled ? "startAutoAnalysis" : "stopAutoAnalysis",
            })
            .catch(() => {});
        });

        state.settings.autoAnalysis = enabled;
        saveSettings();
        });
    }
    document
        .getElementById("autoNotifications")
    ...
    const mainPhilosopher =
        winner.supporters && winner.supporters.length > 0
    ...
    const winner = data.winner;
    const quoteText =
        winner.quote?.quote || winner.quote?.text || "Citation non disponible";
    const quoteSource = winner.quote?.source || "Auteur inconnu";

    const entry = {
        quote: quoteText,
        source: quoteSource,
        context: context.context || "Sans contexte",
        timestamp: Date.now(),
        consensus: data.consensus?.quorum_percentage || 0,
    };
    ...
    container.innerHTML = state.history
        ...
        const sourceText =
            entry.quote?.source || entry.source || "Auteur inconnu";
        const contextText = entry.context || "Sans contexte";
    ...
    const autoNotifToggle = document.getElementById("autoNotifications");
    const autoAnalysisToggle = document.getElementById("autoAnalysis");

    if (autoNotifToggle) {
        autoNotifToggle.checked = state.settings.autoNotifications;
    }
    if (autoAnalysisToggle) {
        autoAnalysisToggle.checked = state.settings.autoAnalysis !== false;
    }
    }
